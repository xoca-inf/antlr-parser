...
nonTerminalRule[Grammar grammar]: name inheritableAttr attr RETURNS
    rule[grammar, new Grammar.NonTerminal($name.text, $attr.text, $inheritableAttr.text)]
    (OR rule[grammar, new Grammar.NonTerminal($name.text, $attr.text, $inheritableAttr.text)])* SEMICOLON
    ;
...
List<Grammar.Entry<String, String>> entryList = new ArrayList<>();
    ...
    (name inheritableAttr? { entryList.add(new Grammar.Entry($name.text, $inheritableAttr.text)); })+
